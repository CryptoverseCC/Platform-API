...
to contain only elements NOT connected with airdrop identified by given id,
...
def run(conn_mgr, input, id, **ignore):
    ...
    return {
        "items": [item for item in input["items"] if item["address"] not in addresses]
    }
