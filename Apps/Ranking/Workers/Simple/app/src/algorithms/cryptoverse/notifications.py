...
MATCH (claim:Claim)-[:ABOUT]->(aboutClaim:Claim)<-[:ABOUT]-(myClaim:Claim)-[:CONTEXT]->(myContext:Entity),
	(aboutClaim)<-[:AUTHORED]-(aboutIdentity),
    (package)<-[:IN]-(claim)<-[:AUTHORED]-(identity),
    (myPackage)<-[:IN]-(myClaim)<-[:AUTHORED]-(myIdentity:Identity)
WHERE myContext.id IN {ids}
    AND io.userfeeds.erc721.isValidClaim(myClaim)
    AND NOT /*like*/ (claim)-[:TARGET]->(:Claim)
...
def run(conn_mgr, input, **params):
    ...
    return {"items": about_me + targeting_me + my_likes + replies_to_me + replies_after_me}
